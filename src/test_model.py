...
def test_model():
    ...
    # 2. Gọi mô hình và lắp 'não' (Epoch 24) vào
    model = AmodalSwinUNet().to(DEVICE)
    weight_path = '../checkpoints/swin_amodal_epoch_24.pth'
    ...
    input_tensor, target_mask = dataset[idx]
    
    # Đóng gói thành Batch (1, 4, 224, 224)
    input_batch = input_tensor.unsqueeze(0).to(DEVICE)

    # 5. YÊU CẦU AI TƯỞNG TƯỢNG VÀ VẼ
    print(f"Đang xử lý ảnh thứ {idx}...")
    with torch.no_grad():
    ...
    axes[2].set_title("3. U-Net Tưởng Tượng (Epoch 24)")
    axes[2].axis('off')
